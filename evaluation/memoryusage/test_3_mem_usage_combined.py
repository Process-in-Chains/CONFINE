...
df_simulation = pd.read_csv(
    '/Users/luca/Documents/PythonProjects/TEE_Evaluation/test_segment_size/test_simulation_1k/segsize_2/test_2_1.csv', decimal='.', header=0,)

df_sepsis = pd.read_csv(
    '/Users/luca/Documents/PythonProjects/TEE_Evaluation/test_segment_size/test_sepsis/segsize_2/test_2_1.csv', decimal='.', header=0,)

df_volvo = pd.read_csv(
    '/Users/luca/Documents/PythonProjects/TEE_Evaluation/test_segment_size/test_volvo/segsize_2/test_2_1.csv', decimal='.', header=0,)


# Convert in datetime
df_simulation['Timestamp'] = df_simulation['Timestamp'].apply(lambda x: datetime.utcfromtimestamp(x))
df_sepsis['Timestamp'] = df_sepsis['Timestamp'].apply(lambda x: datetime.utcfromtimestamp(x))
df_volvo['Timestamp'] = df_volvo['Timestamp'].apply(lambda x: datetime.utcfromtimestamp(x))
...
plt.plot(result['Durata Normalizzata'], result['Memory usage (MB)'], label='Motivating scenario', color='skyblue', linewidth=5, marker = 'p', markersize=8, alpha = 1)

# Create a line plot for the dataset volvo
plt.plot(result_volvo['Durata Normalizzata'], result_volvo['Memory usage (MB)'], label='BPIC 2013', color='tomato', linewidth=5, marker = 'v', markersize=8, alpha = 0.8)

# Create a line plot for the dataset sepsis
plt.plot(result_sepsis['Durata Normalizzata'], result_sepsis['Memory usage (MB)'], label='Sepsis', color='mediumaquamarine', linewidth=5, marker='*', markersize=8, alpha = 0.9)
...
plt.xlim([0, 100])
plt.ylim([0, df_volvo['Memory usage (MB)'].max()])
